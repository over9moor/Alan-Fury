...
/// <summary>
/// –азмещает объекты через GPU Instancing в один draw call на тип объекта
/// вместо создани€ отдельного GameObject на каждый экземпл€р.
///
/// ќтличи€ от ObjectPlacer:
/// - хранит трансформы экземпл€ров в виде матриц (нет реальных объектов в сцене)
...
///
/// ћинус: объекты не участвуют в физике и не имеют коллайдеров.
/// ƒл€ интерактивных объектов используйте ObjectPlacer (деревь€, камни),
/// а тут Ч фон (трава, мелкие детали).
/// </summary>
public class InstancedObjectPlacer : MonoBehaviour
{
    ...
    public class InstancedObjectType
    {
        ...
        [Header("ѕлотность")]
        [Tooltip(" аждые сколько €чеек ставить один объект (1 = один объект на €чейку)")]
        public int cellsPerObject = 1;
        [Tooltip("„ем больше Ч тем плотнее засев (0..1)")]
        [Range(0f, 1f)] public float density = 0.3f;
        public float minHeight = 0f;
        public float maxHeight = 1f;

        [Header("—мещение в пределах €чейки")]
        [Tooltip("–азброс позиции внутри €чейки (0 = строго в центре)")]
        ...
        // ƒанные дл€ отрисовки (заполн€ютс€ при генерации)
    ...
    [Header("ќтступ от кра€ (в €чейках)")]
    [Tooltip("Ќе спавнить объекты в приграничной полосе. ~совпадает с толщиной тумана при tileSize=4.")]
    ...
    public void PlaceAllObjects()
    {
        ...
        foreach (var objType in objectTypes)
        {
            ...
            {
                Debug.LogWarning($"InstancedObjectPlacer: '{objType.name}' Ч нет меша или материала.");
                continue;
            }

            PlaceType(objType, w, d, ts, origin);
            Debug.Log($"InstancedObjectPlacer: '{objType.name}' Ч {objType.matrices.Count} экземпл€ров");
        }
    ...
    {
        // ѕроходим карту с шагом cellsPerObject
        ...
        for (int x = 0; x < w; x += step)
        {
            ...
            {
                // Ќе спавнить в приграничной полосе (пр€четс€ под туман)
    ...
    /// <summary>
    /// ¬озвращает центр €чейки в мировых координатах (удобно дл€ NPC и логики).
    /// </summary>
    ...
    /// <summary>
    /// ¬озвращает индекс €чейки по мировой позиции.
    /// </summary>
    ...
    void OnDrawGizmosSelected()
    {
        ...
        // –исуем сетку €чеек (ограниченно, чтобы не подвесить редактор)
